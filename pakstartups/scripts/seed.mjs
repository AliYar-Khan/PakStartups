...
// Initialize Admin SDK using Application Default Credentials (firebase login sets these)
let app;
try {
  const svcJson = process.env.FIREBASE_SERVICE_ACCOUNT_JSON;
  if (svcJson && svcJson !== "{}") {
    const { default: adminPkg } = await import("firebase-admin/app");
    app = adminPkg.initializeApp({ credential: cert(JSON.parse(svcJson)) });
  } else {
    const { default: adminPkg } = await import("firebase-admin/app");
    // Use the project ID from .env so ADC works with firebase CLI credentials
    const projectId = process.env.NEXT_PUBLIC_FIREBASE_PROJECT_ID;
    app = adminPkg.initializeApp({
  ...
  // fallback: just init with projectId, relies on GOOGLE_APPLICATION_CREDENTIALS
  const { default: adminPkg } = await import("firebase-admin/app");
  app = adminPkg.initializeApp({
...
async function seedIfEmpty(col, items) {
  ...
  for (const item of items) {
    ...
    batch.set(ref, { ...item, createdAt: Timestamp.now(), updatedAt: Timestamp.now() });
  }
  await batch.commit();
...
await seedIfEmpty("events", [
  { title: "Friday Founder Pitch Night #24", desc: "Watch 5 curated startups pitch to a panel of local and international investors.", type: "PITCHING", location: "Online · Zoom", isOnline: true, organizerId: "seed", organizerName: "PakStartups Lab", dateTs: futureDate(7), dateLabel: "Friday · 8:00 PM PKT", rsvpCount: 284, status: "approved", isFeatured: true },
...
await seedIfEmpty("b2bDemands", [
  ...
  { title: "Rebranding & Visual Identity Package", desc: "Complete brand overhaul including logo, typography, and marketing collateral.", category: "Design", tags: ["EdTech","Scale-up"], budget: "PKR 100k–150k", deadline: "21 Days", ownerId: "seed", ownerName: "Hina Khan, Ops Lead", ownerAvatar: "/images/image-009.jpg", icon: "brush", status: "active" },
